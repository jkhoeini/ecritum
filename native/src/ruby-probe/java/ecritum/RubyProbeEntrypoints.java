...
public final class RubyProbeEntrypoints {
    private RubyProbeEntrypoints() {
    }
    ...
    @CEntryPoint(name = "ecritum_graal_eval_ruby_probe")
    public static int ecritumGraalEvalRubyProbe(
        ...
        try {
            ...
            return writeResult(
                BackendResultCodec.encode(RubyProbeEvaluator.evaluate(sourceText, sourceNameText)),
                outBuffer,
                outBufferSize,
                outBytesWritten
            );
        } catch (Throwable ex) {
            SciEvalResult failure = SciEvalResult.internalError("ruby", "", "ruby probe backend failed");
