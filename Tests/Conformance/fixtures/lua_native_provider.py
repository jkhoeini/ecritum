...
class LuaSession(native.RuntimeSession):
    def eval(self, source):
        language, language_keepalive = native.make_string_view("lua")
        source_name, source_name_keepalive = native.make_string_view("conformance.lua")
...
def pass_result(case_id, actual):
    return {"caseId": case_id, "status": "pass", "reason": "packaged Lua runtime matched expected behavior", "actual": actual}
...
def pending_result(case):
    ...
    return {
        ...
        "reason": "lua native provider lacks capabilities: " + ", ".join(missing),
    }
...
def run_case(case, session):
    ...
    if case_id == "eval.scalar":
        result = session.eval("return 42")
        return pass_result(case_id, {"value": result["value"]})
    if case_id == "eval.array":
        result = session.eval('return {1, "two", true}')
        return pass_result(case_id, {"value": result["value"]})
    if case_id == "eval.object":
        result = session.eval("return {answer = 42}")
        return pass_result(case_id, {"value": result["value"]})
    if case_id == "error.script_to_structured_error":
        result = session.eval('error("boom")')
    ...
    if case_id == "host.host_function_returns_value":
        result = session.eval("return ecritum.app.answer()")
        return pass_result(case_id, {"value": result["value"]})
    ...
    if case_id == "stdlib.json.roundtrip":
        result = session.eval("return ecritum.json.writeString({b = 2, a = 1})")
        return pass_result(case_id, {"status": "ECRITUM_OK", "value": result["value"]})
    if case_id == "stdlib.time.parse_format":
        result = session.eval("return ecritum.time.formatInstant(ecritum.time.parseInstant('2026-06-05T00:00:00Z'))")
        return pass_result(case_id, {"status": "ECRITUM_OK", "value": result["value"]})
    if case_id == "stdlib.time.now_default_denied":
        result = session.eval("return ecritum.time.now()")
        return pass_result(case_id, {
            "status": result["status"],
            "category": result["category"],
            "operation": result["operation"],
    ...
    if case_id == "stdlib.fs.default_denied":
        result = session.eval("return ecritum.fs.readText('/tmp/ecritum-conformance-denied')")
        return pass_result(case_id, {
            "status": result["status"],
            "category": result["category"],
            "operation": result["operation"],
    ...
    if case_id == "stdlib.http.default_denied":
        result = session.eval("return ecritum.http.request({url = 'https://example.com'})")
        return pass_result(case_id, {
            "status": result["status"],
            "category": result["category"],
            "operation": result["operation"],
    ...
    if case_id == "timeout.long_running_script":
        result = session.eval("while true do end")
        return pass_result(case_id, {
            "status": result["status"],
            "category": result["category"],
            "operation": result["operation"],
    ...
    if case_id in {"stdlib.fs.allowed_root_inside", "stdlib.fs.allowed_root_outside_denied"}:
        with tempfile.TemporaryDirectory(prefix="ecritum-conformance-") as root_name:
            ...
            try:
                with LuaSession(session.lib, runtime_config_with_read_root(root)) as fs_session:
                    if case_id == "stdlib.fs.allowed_root_inside":
                        result = fs_session.eval(f"return ecritum.fs.readText({quoted_lua_string(inside)})")
                        return pass_result(case_id, {"status": "ECRITUM_OK", "value": result["value"]})
                    result = fs_session.eval(f"return ecritum.fs.readText({quoted_lua_string(outside)})")
                    return pass_result(case_id, {
                        "status": result["status"],
                        "category": result["category"],
                        "operation": result["operation"],
...
def missing_artifact_response(cases):
    return {
        "protocolVersion": 1,
        "provider": {"id": "lua-native-missing-artifact", "capabilities": []},
        "results": [
            {
                "caseId": case["caseId"],
                "status": "pending_capability",
...
def main():
    ...
    print(json.dumps({
        "protocolVersion": 1,
        "provider": {"id": "lua-native-xcframework", "capabilities": CAPABILITIES},
        "results": results,
    }, sort_keys=True))
    return 0
