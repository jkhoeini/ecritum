...
class RubySession(native.RuntimeSession):
    def eval(self, source):
        language, language_keepalive = native.make_string_view("ruby")
        source_name, source_name_keepalive = native.make_string_view("conformance.rb")
...
def pass_result(case_id, actual):
    return {"caseId": case_id, "status": "pass", "reason": "packaged Ruby runtime matched expected behavior", "actual": actual}
...
def pending_result(case):
    ...
    return {
        ...
        "reason": "ruby native provider lacks capabilities: " + ", ".join(missing),
    }
...
def run_case(case, session):
    ...
    if case_id == "error.script_to_structured_error":
        result = session.eval("raise 'boom'")
    ...
    if case_id == "timeout.long_running_script":
        with RubySession(session.lib, runtime_config_with_timeout(1_000_000)) as timeout_session:
            result = timeout_session.eval("loop {}")
        return pass_result(case_id, {
            "status": result["status"],
            "category": result["category"],
            "operation": result["operation"],
    ...
    if case_id in {"stdlib.fs.allowed_root_inside", "stdlib.fs.allowed_root_outside_denied"}:
        with tempfile.TemporaryDirectory(prefix="ecritum-conformance-") as root_name:
            ...
            try:
                with RubySession(session.lib, runtime_config_with_read_root(root)) as fs_session:
...
def is_ruby_runtime_unavailable(exc):
    message = str(exc)
...
def main():
    ...
    if lib is None:
        print(json.dumps(pending_all_response(
            "ruby-native-missing-artifact",
            "missing dist/local/EcritumRuntime.xcframework",
            cases,
        ), sort_keys=True))
        return 0

    session = RubySession(lib)
    try:
        try:
            session.eval("0")
        except RuntimeError as exc:
            if not is_ruby_runtime_unavailable(exc):
                raise
            print(json.dumps(pending_all_response(
                "ruby-native-runtime-not-wired",
                "packaged runtime does not yet support ruby through ecritum_eval_start",
                cases,
            ), sort_keys=True))
            return 0
    ...
    print(json.dumps({
        "protocolVersion": 1,
        "provider": {"id": "ruby-native-xcframework", "capabilities": CAPABILITIES},
        "results": results,
    }, sort_keys=True))
    return 0
